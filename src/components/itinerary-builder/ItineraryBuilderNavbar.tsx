...
export function ItineraryBuilderNavbar() {
  return (
    <motion.header
    ...
    >
      <div className="mx-auto flex h-16 max-w-[1800px] items-center justify-between gap-4 px-4 md:h-[4rem] md:px-8">
        <div className="flex items-center gap-4">
          ...
          <Link
            to="/plan"
            className="hidden text-xs font-medium uppercase tracking-[0.18em] text-traveloop-steel transition-colors hover:text-traveloop-ice sm:inline"
          >
            ← Plan trip
          </Link>
        </div>
        ...
        <nav className="hidden items-center gap-1.5 lg:flex">
          {[
            { to: '/journeys', label: 'User listing' },
            { to: '/discover', label: 'Explore' },
            { to: '/itinerary/view', label: 'View itinerary' },
            { to: '/packing-checklist', label: 'Packing' },
            { to: '/trip-notes', label: 'Trip notes' },
            { to: '/expense-billing', label: 'Billing' },
          ].map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className="rounded-full border border-traveloop-slate/35 bg-white/[0.03] px-3 py-1.5 text-xs font-medium text-traveloop-ice/80 transition hover:border-traveloop-sky/45 hover:bg-traveloop-sky/10 hover:text-white"
            >
              {item.label}
            </Link>
          ))}
