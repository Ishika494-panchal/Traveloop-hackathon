...
import { Bell, BarChart3, Settings, Users } from 'lucide-react'
    ...
    >
      <div className="mx-auto flex h-16 max-w-[1600px] items-center justify-between px-4 md:h-[4.25rem] md:px-8">
        <Link to="/dashboard" className="group flex items-center gap-2" aria-label="Traveloop home">
          ...
          <span className="bg-gradient-to-r from-traveloop-ice via-traveloop-sky to-traveloop-steel bg-clip-text text-lg font-semibold tracking-tight text-transparent drop-shadow-[0_0_18px_rgba(136,189,242,0.35)]">
            Traveloop
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <Link
            to="/"
            className="hidden rounded-lg border border-traveloop-sky/25 px-3 py-1.5 text-xs font-medium text-traveloop-ice/80 transition-colors hover:border-traveloop-ice/40 hover:text-traveloop-ice sm:inline-block"
          >
            Sign in
          </Link>
          {[
            { icon: BarChart3, label: 'Admin analytics', to: '/admin-analytics' },
            { icon: Users, label: 'Community', to: '/community' },
            { icon: Bell, label: 'Notifications' },
            { icon: Settings, label: 'Settings' },
          ].map(({ icon: Icon, label, to }) =>
            to ? (
              <motion.div key={label} variants={item} initial="rest" whileHover="hover" whileTap="tap">
                <Link
                  to={to}
                  aria-label={label}
